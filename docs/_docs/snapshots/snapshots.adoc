...
To list snapshots created on cluster, use the following command.
...
Node consistent-id [node-uuid]:
  snapshot_name: <size>
  snapshot_name: CREATING (DELETING)
----
...
* If a snapshot is in a transient state (being created or deleted), its size is not reported.
* Snapshot is being detected by its metadata. If metadata is missing or cannot be read, snapshot directory is ignored.
* When snapshot listing and size calculating, no additional snapshot validation is done. Files, its' headers aren't read,
no checksums are calculated, etc.
